<template>
  <div>
    ...
    <div v-if="initDataState === 'FINISHED_OK'" class="post-form-wrapper">
      <pre v-if="false">{{ vuexFormGetValues(formId) }}</pre>
      <form @submit.prevent>
        <textarea-autosize
          maxlength="250"
...
export default {
  ...
  created() {
    ...
    this.debouncedAutoSaveAsDraft = debounce(this.autoSaveAsDraft);
  ...
  */
  ...
  methods: {
    ...
    savePost(status, operation) {
      if (!["CREATE", "UPDATE"].includes(this.getCurrentOperation())) {
        console.log("saving post aborted, not a CREATE or UPDATE route");
        return;
      }
    ...
    detectChanges() {
      ...
      const formValues = vuexFormGetValues(formId);
